class FrontendAdmin::WidgetsController < ApplicationController
  ...
  def create
    ...
    management = "<div class='management'><a href='#edit'>+</a><a href='#delete'>X</a></div>".html_safe
    render :js=> "$('##{placeholder} li[data-id=registered_widget_#{id}]').html('#{management + content}').attr('data-id', 'widget_#{w.id}').addClass('widget-wrap');"
  end
